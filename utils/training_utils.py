...
def read_data(data_dir, duration=None, labels=None):
    ...
    for filename in files:
        ...
        # i = 0
        # for session in binned_sessions:
        #     i = i+1
        #     print("session no:", i, " # connections ", len(session))
        #     for key, packets in session.items():
        #        print((key,), len(packets))

        print("# of binned_sessions",len(binned_sessions)) # alshaboti
		
        # Get the capture source from the binned sessions
        capture_source = get_source(binned_sessions)
        #print(get_source(binned_sessions), get_source(binned_sessions, "IP"))
